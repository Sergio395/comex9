...
class ComexOperation:
    def __init__(self, fecha_ingreso, referencia, otorgante, monto_total, proporcion_del_total_por_fc, moneda, importe_factura,
                 ...
                 doc_ajuste, rc, op1, vta_bonos_usd, p_pendientes, ret_ext):
        ...
        self.rc = rc
        self.op1 = op1
        self.vta_bonos_usd = vta_bonos_usd
        self.p_pendientes = p_pendientes
...
class ComexApp:
        ...
        if os.path.exists(self.file_path_all_fields):
            ...
            for _, row in df.iterrows():
                operacion = ComexOperation(
                    ...
                    row['Doc Ajuste'], row['RC'], row['Op1'], row['Vta Bonos USD'], row['$ Pendientes'], row['Ret Ext']
                )
    ...
    def guardar_operacion(self):
        ...
        operacion = ComexOperation(
            ...
            input("Doc Ajuste: "), input("RC: "), input("Op1: "), input("Vta Bonos USD: "), input("$ Pendientes: "), input("Ret Ext: ")
        )
        self.data.append(operacion)
        ...
        print(f"$ Pendientes:{operacion.p_pendientes}")
    ...
    def modificar_operacion(self):
        ...
        operacion.vta_bonos_usd = input(f"vta_bonos_usd ({operacion.vta_bonos_usd}): ") or operacion.vta_bonos_usd
        operacion.p_pendientes = input(f"$ pendientes ({operacion.p_pendientes}): ") or operacion.p_pendientes
    ...
    def listar_operaciones(self):
        for operacion in self.data:
            ...
            print(f"Vta Bonos USD: {operacion.vta_bonos_usd}")
            print(f"$ Pendientes: {operacion.p_pendientes}")
    ...
    def export_to_excel(self):
        all_fields_data = {
            ...
            "Vta Bonos USD": [op.vta_bonos_usd for op in self.data],
            "$ Pendientes": [op.p_pendientes for op in self.data],
            ...
            if 'Referencia' in df.columns:
                ...
                if not matching_rows.empty:
                    ...
                    data_to_export = {
                        ...
                        "Vta Bonos USD":  [row['Vta Bonos USD']],
